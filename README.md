Building a Data-Driven Future: Analysing EV Infrastructure through an ETL Pipeline

An end-to-end ETL and analytics pipeline for U.S. electric vehicle (EV) infrastructure data, built with Dagster, MongoDB, PostgreSQL, and Python. The pipeline extracts live charging-station data from the OpenChargeMap API and combines it with EV registration and fuel-station datasets to analyse adoption trends, charging infrastructure distribution, and forecast future growth using Facebook's Prophet model.
